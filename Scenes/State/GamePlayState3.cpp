...
void GamePlayState3::Initialize()
{
	//Basic
	camera_ = new Camera();
	...
	EnemySpawn(Vector3(-31, 0, -4), 2, Vector3{ 0.1f,0.0f,0.1f });
	EnemySpawn(Vector3(-21, 0, 7), 3, Vector3{ 0.1f,0.0f,0.1f });
	EnemySpawn(Vector3(0, 0, 14), 1, Vector3{ -0.1f,0.0f,-0.1f });
	EnemySpawn(Vector3(-5, 0, 0), 2, Vector3{ -0.1f,0.0f,-0.1f });
	EnemySpawn(Vector3(-5, 0, -14), 3, Vector3{ 0.1f,0.0f,-0.1f });
...
void GamePlayState3::EnemySpawn(const Vector3& position, int moveMode, Vector3 moveSpeed) {
	enemyCount += 1;
	enemy_ = new Enemy();
...
void GamePlayState3::WallSpawn(const Vector3& position) {
	wall_ = new Wall();
